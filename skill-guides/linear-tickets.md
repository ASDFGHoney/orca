...
`orca linear list` and `orca linear project list` follow the same omitted-limit contract: they walk every provider page, return `meta.limit: null`, and set `truncated`/`meta.hasMore` only when a safety backstop stops the walk. An explicit `--limit <n>` is preserved as the caller cap. `orca linear search` stays capped at 50; JSON reports the provider's pre-pagination `totalCount`, and truncated human output says `truncated: showing N of M`. Older hosts may omit `totalCount`, in which case output falls back to `truncated: showing N`. Project JSON `priorityLabel` stays Linear's title-case provider string.

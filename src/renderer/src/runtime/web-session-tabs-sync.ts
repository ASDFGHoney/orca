...
function createWebSessionTabsNotificationLifecycle(
  environmentId: string,
  options: { seedTrackedWorktrees: boolean }
): WebSessionTabsNotificationReconciler {
  return createWebSessionTabsNotificationReconciler({
    trackedWorktrees: options.seedTrackedWorktrees
      ? getTrackedWebSessionTabsWorktrees(environmentId)
      : [],
    acceptsSnapshot: (snapshot) => isHostMirroredWorktree(snapshot.worktree),
...
function getWebSessionTabsNotificationLifecycle(
  ...
  const lifecycle = createWebSessionTabsNotificationLifecycle(environmentId, {
    // Why: replacement pairings cannot inherit notification eligibility during effect cleanup.
    seedTrackedWorktrees: existing === undefined
  })
...
export function applyWebSessionTabsStorePatch(
  ...
  for (const { environmentId, snapshot, decision } of hostMirrorVerdict.frames) {
    if (decision.apply) {
      getWebSessionTabsNotificationLifecycle(environmentId).markSnapshotCommitted(snapshot)
    }
  }
...
export function useWebSessionTabsSync(): void {
  ...
  useEffect(() => {
    ...
    const beginVisibilityResume = ({
      ...
      const resumedNotificationEnvironmentIds = new Set<string>()
      ...
      for (const index of restartingSpecIndexes) {
        ...
        if (environmentId) {
          resumedNotificationEnvironmentIds.add(environmentId)
      ...
      for (const environmentId of resumedNotificationEnvironmentIds) {

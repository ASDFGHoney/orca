...
export type WorktreeMetaPersistenceOwner = {
  settings: AppState['settings']
  executionHostId: ExecutionHostId
}

export async function persistWorktreeMeta(
  owner: WorktreeMetaPersistenceOwner,
  worktreeId: string,
  updates: Partial<WorktreeMeta>
): Promise<void> {
  const mutation = beginPassiveWorktreeMetaMutation(owner.executionHostId, worktreeId, updates)
  try {
    const target = getActiveRuntimeTarget(owner.settings)

...
describe('matchWorkspaceBoardWorktrees', () => {
  ...
  it('matches only the host row whose branch contains the query', () => {
    ...
    const views = buildWorkspaceKanbanLaneViews({
      worktreesByStatus: new Map([['todo', [local, remote]]]),
      matchingWorktreeIds: identities(local)
    })
    expect(views.get('todo')?.items).toEqual([local])
  })
})

...
describe('MacosSystemSleepAssertion isHolding', () => {
  it('reports nothing held before a start', () => {
    const assertion = new MacosSystemSleepAssertion({
      logger: createLogger(),
      platform: 'darwin',
    ...
    expect(assertion.isHolding()).toBe(false)
  })

  it('reports held after a start and nothing held after a stop', () => {
    const child = new FakeCaffeinateProcess()
    const assertion = new MacosSystemSleepAssertion({
      logger: createLogger(),
      platform: 'darwin',
    ...
    assertion.start('status-change')
    expect(assertion.isHolding()).toBe(true)

    assertion.stop('settings-change')
    // The router releases the other engine on this answer, so a stopped
    // caffeinate must never still claim to be holding.
    expect(assertion.isHolding()).toBe(false)
  })

  it('stops claiming to hold once the child reports an error', () => {
    const child = new FakeCaffeinateProcess()
    const assertion = new MacosSystemSleepAssertion({
      logger: createLogger(),
      platform: 'darwin',

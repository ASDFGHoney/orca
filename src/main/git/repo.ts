...
import { scanGitMarkerSync, resolveRealPathSync } from './repository-marker-detection'
...
/** Only a clean pair of negative Git answers definitively means non-repository. */
function probeGitRepo(path: string): GitRepoProbeResult {
  ...
  try {
    const bareRepo = gitExecFileSync(['rev-parse', '--is-bare-repository'], { cwd: path }).trim()
    if (bareRepo === 'true') {
      return 'repo'
    }
    if (bareRepo !== 'false') {
...
export function getGitRepoRoot(path: string): string {
  try {
    ...
    // Fall through to the validated filesystem marker and then preserve the original path.
  }
  const markerScan = scanGitMarkerSync(path)
  return markerScan.status === 'valid'
    ? normalizeGitRepoRootForInputPath(path, markerScan.rootPath)
    : path
}

/** Return the main-checkout path when `path` is a linked worktree, otherwise null. */
export function getLinkedWorktreeMainRepoRoot(path: string): string | null {
  try {
    ...
    const absoluteCommonDir =
      resolveRealPathSync(resolve(path, commonDir)) ?? resolve(path, commonDir)
    const absoluteGitDir = resolveRealPathSync(resolve(path, gitDir)) ?? resolve(path, gitDir)
    if (absoluteGitDir === absoluteCommonDir || basename(absoluteCommonDir) !== '.git') {
      return null
    }
...
/** Get a human-readable repository name from its path. */

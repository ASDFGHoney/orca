...
export async function stageFile(
  ...
  try {
    await gitExecFileAsync(
      ['add', '--', literalGitPathspec(filePath, options)],
...
export async function unstageFile(
  ...
  invalidateGitReadCaches()
  try {
    await gitExecFileAsync(['restore', '--staged', '--', literalGitPathspec(filePath, options)], {
      ...gitOptionsForWorktree(worktreePath, options)
    })
  } finally {
    invalidateGitReadCaches()
...
export async function bulkStageFiles(
  ...
  try {
    for (let i = 0; i < filePaths.length; i += BULK_CHUNK_SIZE) {
      ...
      await gitExecFileAsync(
        ['add', '--', ...chunk.map((filePath) => literalGitPathspec(filePath, options))],
        gitOptionsForWorktree(worktreePath, options)
      )
    }
  } finally {

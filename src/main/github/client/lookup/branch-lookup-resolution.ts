...
import { ghRepoExecOptions, type OwnerRepo } from '../../gh-utils'
import { isCommitPartOfMergedPR, type MergedPRCommitMembership } from '../../merged-pr-commit-membership'
import { shouldHideNonOpenReviewOnDefaultBranch } from '../../../source-control/repo-default-branch'
import { getGitHubApiRepositoryForRemote, resolveGitHubApiRepositoryCandidates } from '../../github-api-repository'
...
import { derivePullRequestMergeable, isMergedImplicitPR, shouldHideMergedImplicitPR, getCurrentHeadOid, type PullRequestLookupData, type GitHubPRBranchLookupOptions } from './pull-request-lookup-data'
...
export async function resolvePRForBranchOutcome(input: {
  ...
  const { repoPath, branchName, linkedPRNumber, connectionId, fallbackPRNumber, options, localGitOptions, ghOptions, executionScope } = input

...
// Why: keyed per owner (not a process-global flag) so one owner's capability gap doesn't poison others that DO support Issue.parent.
const parentFieldRetriedByOwner = new Map<string, true>()
const parentFieldWarningLoggedByOwner = new Map<string, true>()
// Why: concurrent item fetches for one owner share the capability probe.
export const parentFieldProbeInFlight = new Map<string, Promise<void>>()

// GHES owners are a separate namespace and capability surface from github.com owners.
export function ownerScopeKey(
  owner: string,
  ownerType: GitHubProjectOwnerType,
  host?: string
): string {
  return `${owner}\u0000${ownerType}\u0000${githubProjectHost(host)}`
}

/**
 * Files that still import `node:child_process` directly instead of going
 * through `src/shared/child-process/run-process.ts`.
 *
 * This list is a RATCHET: entries may only be removed. Adding one means a new
 * spawn site is once again deciding console visibility, argument quoting and
 * `.cmd` handling for itself — the gap that produced stolen keyboard focus
 * (#14543, #10488), mangled agent prompts (#8793, #11374, #6694) and process
 * trees nothing owns.
 *
 * Migrating a file means replacing its `spawn`/`execFile` calls with
 * `runProcess`/`spawnProcess` and deleting its line here.
 */
export const CHILD_PROCESS_IMPORT_ALLOWLIST: readonly string[] = [
